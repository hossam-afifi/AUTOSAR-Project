...
static void scheduleTxFifo(void)
{
	...
	Can_PduType canPdu;
	Can_ReturnType rVal = E_R_OK;
	imask_t imask;

	canTxPdu = RB_OUTP(canifTx);

	while((NULL != canTxPdu) && (E_R_OK == rVal))
	{
...
static void scheduleRxIndication(uint16 Hrh, Can_IdType CanId, uint8 CanDlc,
	...
	if (CanIf_GetPduMode(channel, &mode) == E_R_OK) {
...
void CanIf_InitController(uint8 Controller, uint8 ConfigurationIndex)
{
  ...
  if (CanIf_GetControllerMode(channel, &mode) == E_R_OK)
  {
    ...
    const PduInfoType *PduInfoPtr)
{
  ...
  if (CanIf_GetControllerMode(channel, &csMode) == E_R_NOT_OK){
    return E_R_NOT_OK;
  }
  ...
  if (CanIf_GetPduMode(channel, &pduMode) == E_R_NOT_OK){
    return E_R_NOT_OK;
  }
