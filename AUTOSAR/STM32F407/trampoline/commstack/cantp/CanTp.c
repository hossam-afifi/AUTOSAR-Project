	...
	*bytesWrittenSuccessfully = 0;
	...
	while ((*bytesWrittenSuccessfully < segmentSize) && (!endLoop)) {
		...
		if (*bytesWrittenSuccessfully < segmentSize ) {
			...
			ret = (rxConfig->PduR_CanTpProvideRxBuffer)(rxConfig->PduR_PduId, rxRuntime->transferTotal, &rxRuntime->pdurBuffer);  /** @req CANTP079 */ /** @req CANTP080 */ /** @req CANTP064 */
			if (ret == BUFREQ_OK) {
...
static bool copySegmentToLocalRxBuffer /*writeDataSegmentToLocalBuffer*/(
		CanTp_ChannelPrivateType *rxRuntime, uint8 *segment,
...
static void sendFlowControlFrame(const CanTp_RxNSduType *rxConfig, CanTp_ChannelPrivateType *rxRuntime, BufReq_ReturnType flowStatus) {
	...
	if (ret != E_R_OK) {
		...
		(rxConfig->PduR_CanTpRxIndication)(rxConfig->PduR_PduId, NTFRSLT_E_NOT_OK);  /** @req CANTP084 */
...
static void handleConsecutiveFrame(const CanTp_RxNSduType *rxConfig,
	...
	if (rxRuntime->iso15765.state == RX_WAIT_CONSECUTIVE_FRAME) {
		...
		if (segmentNumber != (rxRuntime->iso15765.framesHandledCount & SEGMENT_NUMBER_MASK)) {
			...
			(rxConfig->PduR_CanTpRxIndication)(rxConfig->PduR_PduId, NTFRSLT_E_WRONG_SN); /** @req CANTP084 */
			rxRuntime->iso15765.state = IDLE;
			rxRuntime->mode = CANTP_RX_WAIT;
		} else {
			...
			if (ret == BUFREQ_NOT_OK) {
				...
				(rxConfig->PduR_CanTpRxIndication)(rxConfig->PduR_PduId, NTFRSLT_E_NO_BUFFER); /** @req CANTP084 */
				rxRuntime->iso15765.state = IDLE;
				rxRuntime->mode = CANTP_RX_WAIT;
			} else if (ret == BUFREQ_BUSY) {
				...
				if (bytesLeftToTransfer > 0) {
					...
					(rxConfig->PduR_CanTpRxIndication)(rxConfig->PduR_PduId, NTFRSLT_OK); /** @req CANTP084 */
				}
			}
		}
	} else {
...
static BufReq_ReturnType sendNextTxFrame(
	...
	for(; txRuntime->canFrameBuffer.byteCount < MAX_SEGMENT_DATA_SIZE && ret == BUFREQ_OK ;) {
		if(txRuntime->pdurBuffer == 0 || txRuntime->pdurBufferCount == txRuntime->pdurBuffer->SduLength) {
			...
			ret = (txConfig->PduR_CanTpProvideTxBuffer)(txConfig->PduR_PduId, &txRuntime->pdurBuffer, 0);
...
static void handleNextTxFrameSent(
	...
	if (txRuntime->transferTotal <= txRuntime->transferCount) {
		...
		(txConfig->PduR_CanTpTxConfirmation)(txConfig->PduR_PduId, NTFRSLT_OK); /** @req CANTP074 *//** @req CANTP09 *//** @req CANTP204 */
		...
		if (resp == BUFREQ_OK ) {
			...
			(txConfig->PduR_CanTpTxConfirmation)(txConfig->PduR_PduId, NTFRSLT_E_NOT_OK);  /** @req CANTP177 */ /** @req CANTP084 */
			txRuntime->iso15765.state = IDLE;
			txRuntime->mode = CANTP_TX_WAIT;
		}
	} else {
	...
	if ( txRuntime->iso15765.state == TX_WAIT_FLOW_CONTROL ) {
		...
		case ISO15765_FLOW_CONTROL_STATUS_OVFLW:
			...
			for (int i = 0; i < CANTP_NSDU_CONFIG_LIST_SIZE; i++) {
			      ...
			      rxConfig = (CanTp_RxNSduType*) &CanTpConfig.CanTpNSduList[i].configData;
			      break;
			   }
			}
			(rxConfig->PduR_CanTpRxIndication)(txConfig->PduR_PduId, NTFRSLT_E_NOT_OK);
			txRuntime->iso15765.state = IDLE;
			txRuntime->mode = CANTP_TX_WAIT;
			break;
		}
...
static void handleSingleFrame(const CanTp_RxNSduType *rxConfig,
	...
	if (rxRuntime->iso15765.state != IDLE) {
		...
		(rxConfig->PduR_CanTpRxIndication)(rxConfig->PduR_PduId, NTFRSLT_E_NOT_OK);  // Abort current reception, we need to tell the current receiver it has been aborted.
	...
	if (ret == BUFREQ_OK) {
		...
		(rxConfig->PduR_CanTpRxIndication)(rxConfig->PduR_PduId, NTFRSLT_OK);
		rxRuntime->iso15765.state = IDLE;
		rxRuntime->mode = CANTP_RX_WAIT;
	} else if (ret == BUFREQ_BUSY) {
		...
		(rxConfig->PduR_CanTpRxIndication)(rxConfig->PduR_PduId, NTFRSLT_E_NO_BUFFER);
		rxRuntime->iso15765.state = IDLE;
		rxRuntime->mode = CANTP_RX_WAIT;
	}
}
...
static ISO15765FrameType calcRequiredProtocolFrameType(
		const CanTp_TxNSduType *txConfig, CanTp_ChannelPrivateType *txRuntime, bool* isClassic) {
		...
		const PduInfoType *CanTpTxInfoPtr)  /** @req CANTP176 */
{
	...
	if( CanTpConfig.CanTpRxIdList[CanTpTxSduId].CanTpNSduIndex != 0xFFFF ) {
		...
		if (txRuntime->iso15765.state == IDLE) {
			...
			ret = E_R_NOT_OK;  /** @req CANTP123 *//** @req CANTP206 */
		}
	}
...
static bool checkNasNarTimeout(CanTp_ChannelPrivateType *runtimeData) {
	bool ret = false;
		...
		if ( CanTpConfig.CanTpNSduList[i].direction == IS015765_TRANSMIT ) {
			...
			case TX_WAIT_TRANSMIT: {
				...
				if ( ret == BUFREQ_OK ) {
					...
					if (txRuntimeListItem->iso15765.stateTimeoutCount == 0) {
						...
						(txConfigListItem->PduR_CanTpTxConfirmation)(txConfigListItem->PduR_PduId, NTFRSLT_E_NOT_OK); /** @req CANTP204 */
					} else {
			...
			case TX_WAIT_FLOW_CONTROL:
				...
				if (txRuntimeListItem->iso15765.stateTimeoutCount == 0) {
					...
					(txConfigListItem->PduR_CanTpTxConfirmation)(txConfigListItem->PduR_PduId, NTFRSLT_E_NOT_OK); /** @req CANTP204 */ /** @req CANTP185 */
				}
				break;
			case TX_WAIT_TX_CONFIRMATION:
				...
				if (txRuntimeListItem->iso15765.stateTimeoutCount == 0) { /** @req CANTP075 */
					...
					(txConfigListItem->PduR_CanTpTxConfirmation)(txConfigListItem->PduR_PduId, NTFRSLT_E_NOT_OK); /** @req CANTP074 */ /** @req CANTP204 */
				}
				break;
			default:
				break;
			...
			case RX_WAIT_CONSECUTIVE_FRAME: {
				...
				if (rxRuntimeListItem->iso15765.stateTimeoutCount == 0) {
					...
					(rxConfigListItem->PduR_CanTpRxIndication)(rxConfigListItem->PduR_PduId, NTFRSLT_E_NOT_OK);
			...
			case RX_WAIT_SDU_BUFFER: {
				...
				if (rxRuntimeListItem->iso15765.stateTimeoutCount == 0) { /** @req CANTP223 */
					...
					(rxConfigListItem->PduR_CanTpRxIndication)(rxConfigListItem->PduR_PduId, NTFRSLT_E_NOT_OK); /** @req CANTP204 */
					rxRuntimeListItem->iso15765.state = IDLE;
					rxRuntimeListItem->mode = CANTP_RX_WAIT;
				} else { /** @req CANTP222 */
					...
					if (ret == BUFREQ_OK) {
						if ( bytesRemaining > 0 ) {
							...
							(rxConfigListItem->PduR_CanTpRxIndication)(rxConfigListItem->PduR_PduId, NTFRSLT_OK);
							rxRuntimeListItem->iso15765.state = IDLE;
							rxRuntimeListItem->mode = CANTP_RX_WAIT;
						}
					} else if (ret == BUFREQ_NOT_OK ) {
						//PduR_CanTpRxIndication(rxConfigListItem->PduR_PduId, NTFRSLT_E_NOT_OK); /** @req CANTP205 */
						(rxConfigListItem->PduR_CanTpRxIndication)(rxConfigListItem->PduR_PduId, NTFRSLT_E_NOT_OK); /** @req CANTP205 */
						rxRuntimeListItem->iso15765.state = IDLE;
						rxRuntimeListItem->mode = CANTP_RX_WAIT;
					} else if ( ret == BUFREQ_BUSY ) {
